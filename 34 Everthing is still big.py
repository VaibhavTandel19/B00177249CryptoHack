#first we generate big prime number to create RSA public key and then we can encrypt the message  using public key
# then we use Wiener’s attack to attempt get private key and decrypt the message#

!pip install pycryptodome

!pip install git+https://github.com/maddivy/owiener.git

from Crypto.Util.number import getPrime, bytes_to_long, long_to_bytes
import owiener

FLAG = b"crypto{?????????????????????????}"
m = bytes_to_long(FLAG)

def get_huge_RSA():
    p = getPrime(1024)
    q = getPrime(1024)
    N = p * q
    phi = (p - 1) * (q - 1)
    while True:
        d = getPrime(256)
        e = pow(d, -1, phi)
        if e.bit_length() == N.bit_length():
            break
    return N, e

N, e = get_huge_RSA()

c = pow(m, e, N)

print(f'N = {hex(N)}')
print(f'e = {hex(e)}')
print(f'c = {hex(c)}')

N = int('', 16)
e = int('', 16)
c = int('', 16)

d = owiener.attack(e, N)

m = pow(c, d, N)

print(long_to_bytes(m))
